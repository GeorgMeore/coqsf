...
Theorem add_inc_count:
  forall (n: nat) (b: bag), count n (add n b) = 1 + count n b.
Proof.
  intros n b.
...
Theorem bag_count_sum:
  forall b1 b2 n, count n b1 + count n b2 = count n (sum b1 b2).
Proof.
  intros b1 b2 n.
  induction b1.
  - reflexivity.
...
Proof.
  intros f Hf.
  intros n1 n2 H.
  rewrite -> Hf. rewrite <- H. rewrite <- Hf. reflexivity.
Qed.
...
Proof.
  intros. destruct x. rewrite <- eqb_refl with n. reflexivity.
Qed.
(** [] *)
...
(* baz has no elements, since we can't construct any. *)
