...
Definition flat_length {X: Type} (l: list (list X)): nat :=
  fold (fun x a => a + length x) l 0.

Example flat_length_example: flat_length [[1; 2; 3]; [4; 5]] = 5.
Proof. reflexivity. Qed.
...
(*
   Proof by induction.
     The base case of [l = []] is trivial and follows from the definition.

     Now suppose that [l = x :: l'] and [length l' = n'] and [nth_error l' n' = None].

     It follows from the [length] definition that [n = length l = S n'].

     If we expand [nth_error (S n') l] we get [nth_error n' l'] and that is [None] by the
...
Definition exp (n m : cnat) : cnat
  := fun X f x => (m (X->X) (fun g => n X g) f) x.
